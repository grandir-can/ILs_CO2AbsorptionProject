...
def train_evaluate_rf(feature_type='Morgan', target_property='viscosity'):
    # Select data and index paths based on target property
    if target_property == 'CO2':
    ...
    if feature_type == 'Morgan':
        with open('../../data/morgan_features.pkl', 'rb') as f:
            feature_dict = pickle.load(f)
    else:
        raise ValueError("feature_type must be 'Morgan'")
    ...
    output_dir = f'../../data/result/{target_property}/RandomForest_{feature_type}/'
...
if __name__ == '__main__':
    train_evaluate_rf(feature_type='Morgan', target_property='viscosity')
